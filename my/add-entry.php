...
if (isset($_POST['call'])) {
  ...
  try {
    if ($entryType === 'work_experience') {
      ...
      $stmt = $pdo->prepare(
        "INSERT INTO `employers` (`user_id`, `employer`, `job_position_lang_1`, `area`, `country_lang_1`, `start_date`, `end_date`, `is_current`) 
      ...
      // Prepare the SQL statement
      $stmt = $pdo->prepare(
        "INSERT INTO `courses` (`user_id`, `school`, `course_lang_1`, `area`, `country_lang_1`, `start_date`, `end_date`, `is_current`) 
