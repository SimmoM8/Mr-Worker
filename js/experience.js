...
const Experience = {
  ...
  init: function () {
    console.log("Experience.js: Initialization started");
  ...
  initWorkExperience: function () {
    console.log("Initializing Work Experience Page");
  ...
  ajaxRequest: function (url, type, data = {}, onSuccess = () => {}, onError = () => {}) {
    $.ajax({
        url: url,
        type: type,
        data: data,
  ...
  renderExperienceCard: function (experience, call) {
    ...
    <div class="experience_card-container row" id="experience_card_${experience.id}">
      <div class="time-plot col-1">
        <p class="time">${experience.end_date}<br> <i class="fas fa-arrow-up"></i><br>${experience.start_date}</p>
      </div>
      <div class="card experience_card col">
        <div class="experience-header card-colored" id="experience_header_${experience.id}">
          <h3>${experience.title}</h3>
          <p class="sub-heading">${experience.organization}  ●  ${experience.location}</p>
          <div class="menu-icon-container" id="menu_container_${experience.id}">
            <div class="action-buttons slide-in" id="action_buttons_${experience.id}">
              <button class="menu-btn btn-outline-primary edit-experience" data-id="${experience.id}" data-call="${call}">
                <i class="fas fa-pencil-alt"></i> Edit
              </button>
              <button class="menu-btn btn-outline-danger delete-experience" data-id="${experience.id}" data-call="${call}">
  ...
  reorderItems: function (evt, section) {
    const containerId = section === 'work_experience' ? '#employers' : '#courses';
    const updatedOrder = [...document.querySelectorAll(`${containerId} .experience_card-container`)].map((item, index) => ({
  ...
  populateDateDropdowns: function (isEdit, experience = null) {
    // Explicitly convert isEdit to a boolean
    ...
    $('#start_month, #end_month').empty().append('<option value="">Select Month</option>');
    ...
    $('#start_year, #end_year').empty().append('<option value="">Select Year</option>');
    ...
    if (isEdit && experience) {
      if (experience.start_date) {
        const [startYear, startMonth] = experience.start_date.split('-');
      ...
      if (experience.is_current) {
        ...
        if (experience.end_date) {
          const [endYear, endMonth] = experience.end_date.split('-');
    ...
    $('#current_job').off('change').on('change', function () {
      if ($(this).is(':checked')) {
        $('#end-date-container').hide();
      } else {
  ...
  openExperienceModal: function (isEdit = false, experience = null, call = null) {
    ...
    if (call === 'work_experience') {
      ...
      const modalTitle = isEdit ? 'Edit Work Experience' : 'Add Work Experience';

      positionField = isEdit ? experience.job_position : '';
      ...
      $('#organization-field-work input').val(organisationField).prop('disabled', false);
      $('#position-field-work input').val(positionField).prop('disabled', false);
      // Hide and disable
      $('#organization-field-education, #position-field-education').hide();
      $('#organization-field-education input, #position-field-education input').prop('disabled', true);
      ...
      positionField = isEdit ? experience.course : '';
      organisationField = isEdit ? experience.school : '';

      $('#modalTitleExperience').text(modalTitle);
      // Show and enable
      $('#organization-field-education, #position-field-education').show();
      $('#organization-field-education input').val(organisationField).prop('disabled', false);
      $('#position-field-education input').val(positionField).prop('disabled', false);
      // Hide and disable
      $('#organization-field-work, #position-field-work').hide();
      $('#organization-field-work input, #position-field-work input').prop('disabled', true);
    }
  ...
  editSkillItem: function (event) {
    if (!$(event.target).is('input, .delete-point')) {
      ...
      pointText.html(`<input type="text" class="form-control skill-item-input" value="${originalPointValue}">`);
      $('.skill-item-input').focus();

      $('.skill-item-input').off('blur').on('blur', function () {
  ...
  addExperience: function (call) {
    ...
    if (!formData.is_current) {
      ...
      formData.end_date = `${Experience.getMonthAbbreviation(formData.end_month)} ${formData.end_year}`;
    } else {
      // Set end date to current date when "Currently working here" is checked
      const today = new Date();
      formData.end_date = `${Experience.getMonthAbbreviation(`0${today.getMonth() + 1}`.slice(-2))} ${today.getFullYear()}`;
    }

    // Create the formatted start date using month abbreviations
    formData.start_date = `${Experience.getMonthAbbreviation(formData.start_month)} ${formData.start_year}`;
  ...
  addEventListeners: function () {
    ...
    $(document).off('click', '.edit-experience').on('click', '.edit-experience', function (e) {
      e.stopPropagation(); // Prevent click event from affecting other elements
      e.preventDefault();
    ...
    $(document).off('click', '.delete-experience').on('click', '.delete-experience', function (e) {
      e.stopPropagation(); // Prevent click event from affecting other elements
      e.preventDefault();
    ...
    $(document).off('click', '#addEntryBtn').on('click', '#addEntryBtn', function () {
    ...
    $(document).off('click', '#updateEntryBtn').on('click', '#updateEntryBtn', function () {
    ...
    $(document).off('click', '.skill-item').on('click', '.skill-item', Experience.editSkillItem);
    $(document).off('click', '.delete-point').on('click', '.delete-point', Experience.deleteSkillItem);
    $(document).off('click', '#btn-add-experience').on('click', '#btn-add-experience', function () {
    ...
    $(document).off('click', '.experience-header').on('click', '.experience-header', function (e) {
